        ...
        if df[col].dtype == 'object':
            ...
            try:
                ...
                if not numeric_version.isna().all():
                    # If some values are numeric, convert all to string to maintain consistency
                    df[col] = df[col].astype(str)
                else:
                    # If no numeric values, ensure it's string type
                    df[col] = df[col].astype(str)
            except:
                df[col] = df[col].astype(str)
...
def safe_dataframe_display(df, **kwargs):
    ...
    try:
        # First attempt: display as-is
        return st.dataframe(df, **kwargs)
    except Exception as e:
        try:
            # Second attempt: fix types
            df_fixed = df.copy()
            ...
            # Ensure no mixed types in any column
            for col in df_fixed.columns:
                if 'Valeur' in col or 'Value' in col:  # Specifically handle 'Valeur' columns
                    df_fixed[col] = df_fixed[col].astype(str)
            
            return st.dataframe(df_fixed, **kwargs)
            
        except Exception as e2:
            # Third attempt: convert entire dataframe to strings
            try:
                df_string = df.copy()
...
if df is not None:
    ...
    st.session_state.filtered_df = df  # Pour compatibilité avec vos modules
    
    # Sidebar navigation with contexte Fani Maoré (CLEAN VERSION - NO REDUNDANT NAVIGATION)
    with st.sidebar:
        st.markdown("""
    ...
    if page == "🏠 Accueil":
        ...
        # Display a safe version of the dataframe
        ...
        if 'filtered_df' in st.session_state:
            try:
                ...
                with col1:
                    ...
                    # Create a safe statistics display
