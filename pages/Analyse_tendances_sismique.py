...
Ce module fournit une analyse complète des tendances sismiques incluant :
- Tendances saisonnières avec tests statistiques
- Tendances journalières et cycles hebdomadaires
- Tendances à long terme avec régression linéaire
...
def show_analyse_tendances():
    ...
    # Préparer les données avec les composantes temporelles nécessaires
    df_filtered = prepare_temporal_components(df_filtered)
    
    # Séparation
    st.markdown("---")
    
    # Afficher l'analyse selon le type sélectionné
...
def apply_trends_filters_simple(df, annees_range, selected_mois, magnitude_range, profondeur_range, selected_saisons):
    ...
    # Assurer la compatibilité des colonnes
...
def prepare_temporal_components(df):
    ...
    # S'assurer que nous avons les bonnes colonnes temporelles
    if 'Date_dt' in df.columns:
        df['Date'] = df['Date_dt']
    
    # Extraire les composantes temporelles si elles n'existent pas
    ...
    # Ajouter les composantes spéciales pour l'analyse des tendances
    try:
...
def analyser_tendances_saisonnieres(df_filtered):
    ...
    # Créer un dictionnaire pour tous les mois
...
def analyser_tendances_journalieres(df_filtered):
    ...
    # Ajouter les valeurs et calculs
...
def analyser_tendances_long_terme(df_filtered):
    ...
    if len(df_mensuel) > 1:
        ...
        ax.plot(df_mensuel.index, trend_line, color='red', linestyle='--', linewidth=2,
               label=f'Tendance brute (pente={model.slope:.4f}, p={model.pvalue:.4f})')
    ...
    if len(df_mensuel) >= 6:
        ...
        if len(rolling_mean.dropna()) > 1:
            ...
            ax.plot(trend_indices, trend_line_roll, color='purple', linestyle='--', linewidth=2,
                   label=f'Tendance lissée (pente={model_roll.slope:.4f}, p={model_roll.pvalue:.4f})')
            
            # Analyse de la tendance lissée
